class CreateElements {
    ...
    createButtonWrapper() {
      ...
      btnWrapper.className = 'grid__wrapper';
      btnWrapper.id = 'grid__wrapper';
      for (let i = 0; i < 5; i++) {
    ...
    createKeyboardButton(value) {
      ...
      if (value.length === 1 || value === 'space') {
        keyBtn.classList.add('letter__btn');
      } else {
        keyBtn.classList.add('functional__btn');
      }
      ...
      else if (value !== 'space' && keyBtn.className !== 'functional__btn') keyBtn.innerText = value.toUpperCase();
  ...
  class EventFunctions {
    ...
    keyDownEvent(event, creator, text) {
      ...
      if (keyName === 'Shift') {
        ...
        Array.from(document.querySelectorAll('[shift]'))
          .map((item) => item.style.backgroundColor = 'rgb(116, 138, 148)');
        ...
        if (keyName === 'CapsLock') {
          ...
          Array.from(document.querySelectorAll('[alt]')).map((item) => item.style.backgroundColor = 'rgb(116, 138, 148)');
          creator.funcKeys.alt = true;
        } else if (keyName === ' ') keyDefault = 'space';
        else if (!document.getElementById(keyDefault)) return
        const virtBtn = document.getElementById(keyDefault).closest('.functional__btn')
          || document.getElementById(keyDefault).closest('.letter__btn');
        virtBtn.style.backgroundColor = virtBtn.className === 'letter__btn' ? 'rgb(102, 106, 129)' : 'rgb(116, 138, 148)';
    ...
    keyUpEvent(event, creator) {
      ...
      if (keyName === 'Shift') {
        ...
        const virtBtn = document.getElementById(keyLow).closest('.functional__btn')
          || document.getElementById(keyLow).closest('.letter__btn');
        virtBtn.style = '';
      }
    }
    ...
    clickEvent(event, creator, text) {
      text.focus();
      const target = event.target.closest('.letter__btn') || event.target.closest('.functional__btn');
      if (target) {
        if (target.id === 'space') {
          this.addSymbolInside(text, '\u00a0');
        } else if (target.className === 'letter__btn') {
  ...
  const wrapper = document.getElementById('grid__wrapper');
